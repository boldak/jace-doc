...
class CssProcessor {
  ...
  async processAll() {
    ...
    return _.zipObject(sourcePaths, outputUrls);
  }
  ...
  async process(path) {
    ...
    const sourcePath = p.resolve(this.assets, path);
    ...
    return {
      source: p.relative(this.assets, sourcePath),
      output,
   ...
   */
  async getFiles() {
    console.log(`Process ${this.assets} ["**/[^_]*.css"]`)
    return globby("**/[^_]*.css", {
      cwd: this.assets
    });
  }
